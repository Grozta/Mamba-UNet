...
#     classifier_2 = create_model('projector')
    ...
    """
    # x.shape [4,1,224,224]
    x = torch.rand(batch_size,1, args.patch_size[0],args.patch_size[0]).cuda()
    flops, params = profile(model1, inputs=(x, ))
    flops, params = clever_format([flops, params], "%.3f")
    logging.info(f"model--->[flops, params]: {flops},{params}")
    # p.shape [2,4,224,224]
    p = torch.rand(args.labeled_bs, args.num_classes, args.patch_size[0],args.patch_size[0]).cuda()
    flops, params = profile(projector_1, inputs=(p, ))
    logging.info(f"projector--->[flops, params]: {flops},{params}")
...
#             running_comple_loss += comple_loss
            ...
            if iter_num % 50 == 0:
                ...
                # show shuffle image
                image = Jigsaw_batch[idx, 0:1, :, :]
                writer.add_image("train/Jigsaw_batch", image, iter_num)
