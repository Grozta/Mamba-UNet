...
def test_single_volume_for_trainLabel(image, label, net, ema_net, classes, patch_size=[256, 256],args = None):
    image, label = image.squeeze(0).cpu().detach(
    ).numpy(), label.squeeze(0).cpu().detach().numpy()
    prediction = np.zeros_like(label)
    for ind in range(image.shape[0]):
        ...
        with torch.no_grad():
            ...
            if 1 in args.vae_option:
                out = torch.argmax(torch.softmax(ema_net(out)[0], dim=1), dim=1).squeeze(0)
            else:
...
def test_single_volume_for_VAE(image, label, vaenet, eme_net, classes, patch_size=[256, 256],args = None):
    image, label = image.squeeze(0).cpu().detach(
    ).numpy(), label.squeeze(0).cpu().detach().numpy()
    prediction = np.zeros_like(label)
    for ind in range(image.shape[0]):
        ...
        eme_net.eval()
        with torch.no_grad():
            vaenet.forward(input, None, training=False)
            seg_outputs = vaenet.sample(testing=True)
            eme_outputs,_ = eme_net(seg_outputs, None)
            out = torch.argmax(torch.softmax(eme_outputs, dim=1), dim=1).squeeze(0)
            out = out.cpu().detach().numpy()
            pred = zoom(out, (x / patch_size[0], y / patch_size[1]), order=0)
            prediction[ind] = pred
    metric_list = []
